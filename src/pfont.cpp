...
p_private p_fn u32 pfont_round_size_to_sizes_index(u32 size) {
  for (u32 i = 0; i < p_sizeof_array(FONT_SIZES); i++) {
    if (i > 0) {
      if (FONT_SIZES[i] >= size && FONT_SIZES[i - 1] <= size) {
        return i;
      }
    } else {
...
p_fn ALLEGRO_FONT *pfont_get_size(p_font *font, u32 size) {
  ...
  assert(selected_size_index > 0 && font->loaded_sizes[selected_size_index]);
