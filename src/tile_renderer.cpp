...
p_fn err_code tr_tile_map_render(TileMap *map, float mouse_x, float mouse_y, ALLEGRO_KEYBOARD_STATE *state, bool debug) {
  ...
  if (debug) {
    ...
    {
      uint screen_width = al_get_bitmap_width(previous_target);
      uint screen_height = al_get_bitmap_height(previous_target);
      
      float selection_window_size_x = screen_width - 12, selection_window_size_y = 250;
      float SEL_PADDING = 10.f;
      ...
      if (rm_get_image("selection_menu_render", &selection_menu_framebuffer) == ERR_RM_NOT_FOUND || selection_menu_framebuffer == NULL) {
        selection_menu_framebuffer = al_create_bitmap(screen_width - SEL_PADDING * 2, selection_window_size_y);
